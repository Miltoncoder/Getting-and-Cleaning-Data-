...
#Data Processing 
From the original dataset we processed the data in order to 
+ Merge the training and the test sets to create one data set called append_data in run_analysis.R
+ Extract only the measurements on the mean and standard deviation for each measurement stored called append_data_clean in run_analysis.R
+ Create a second, independent tidy data set with the average of each variable for each activity and each subject called summary_data in run_analysis.R. This data frame was stored in results.txt
